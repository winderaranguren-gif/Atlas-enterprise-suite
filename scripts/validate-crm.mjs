...
const modules=[
  ...
  'modules/crm-quote-update.js','modules/crm-communications.js','modules/crm-dashboard.js','modules/crm-search.js','modules/crm-automations.js',
  'modules/crm-rule-runner.js','modules/crm-ui.js'
];
...
for(const table of ['crm_accounts','crm_contacts','crm_pipeline_stages','crm_leads','crm_opportunities','crm_activities','crm_quotes','crm_quote_lines','crm_communications','crm_automation_rules','crm_automation_runs']){
  if(!migration.includes(`CREATE TABLE IF NOT EXISTS ${table}`))throw new Error(`missing_crm_table:${table}`);
}
...
for(const route of ['/api/crm/status','/api/crm/dashboard','/api/crm/pipeline','/api/crm/accounts','/api/crm/contacts','/api/crm/leads','/api/crm/opportunities','/api/crm/activities','/api/crm/quotes','/api/crm/communications'])if(!router.includes(route))throw new Error(`missing_crm_route:${route}`);
