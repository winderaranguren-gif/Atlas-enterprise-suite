...
const securityBefore=await call('/api/security-events?limit=20',{token:managerToken});
mark('Privileged security evidence readable',securityBefore.status===200&&Array.isArray(securityBefore.data.events)&&securityBefore.data.organization_id===org&&securityBefore.data.dba_id===dba,JSON.stringify(securityBefore));
...
const userDirectory=await call('/api/users',{token:viewerToken});
mark('Viewer cannot enumerate user directory',userDirectory.status===403,JSON.stringify(userDirectory));
const audit=await call('/api/audit',{token:viewerToken});
mark('Viewer cannot read audit trail',audit.status===403,JSON.stringify(audit));
const security=await call('/api/security-events',{token:viewerToken});
mark('Viewer cannot read security evidence',security.status===403,JSON.stringify(security));
...
const deniedEvidence=await call('/api/security-events?decision=deny&limit=200',{token:managerToken});
mark('Denied authorization evidence queryable',deniedEvidence.status===200&&Array.isArray(deniedEvidence.data.events)&&deniedEvidence.data.events.some(e=>e.user_id===created.data.user_id&&e.decision==='deny'),JSON.stringify(deniedEvidence));
