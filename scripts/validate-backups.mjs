...
const required=[
 ...
 ['backup schema v2',worker.includes("schema_version:'2'")],
 ...
 ['membership coverage',worker.includes("'atlas_memberships'")],
 ['scoped user coverage',worker.includes("table==='atlas_users'")&&worker.includes('JOIN atlas_memberships m ON m.user_id=u.id')],
 ['scoped credential coverage',worker.includes("table==='atlas_password_credentials'")&&worker.includes('JOIN atlas_memberships m ON m.user_id=c.user_id')],
 ['security-event coverage',worker.includes("'atlas_security_events'")],
 ['ephemeral sessions excluded',worker.includes("'atlas_sessions'")&&worker.includes('EXCLUDED_EPHEMERAL_TABLES')],
 ['activation tokens excluded',worker.includes("'atlas_activation_tokens'")&&worker.includes('EXCLUDED_EPHEMERAL_TABLES')],
 ['bootstrap state excluded',worker.includes("'atlas_bootstrap_state'")&&worker.includes('EXCLUDED_EPHEMERAL_TABLES')],
 ['restore validates exclusions',worker.includes('validExclusions')],
 ['manifest mutations remain scoped',worker.includes("WHERE id=? AND organization_id=? AND dba_id=?")],
