...
for(const check of checks){
  ...
  try{
    const response=await fetch(url,{redirect:'follow',headers:{'user-agent':'ATLAS-Production-Verifier/2.0'},signal:AbortSignal.timeout(15000)});
    const text=await response.text();
    if(response.status!==check.status){console.error(`FAIL ${check.path}: HTTP ${response.status}, expected ${check.status}`);failed=thisFailed=true;continue}
    ...
    if(check.json){let data;try{data=JSON.parse(text)}catch{console.error(`FAIL ${check.path}: invalid JSON`);failed=thisFailed=true;continue}if(!check.json(data)){console.error(`FAIL ${check.path}: production state/identity mismatch`);failed=thisFailed=true}}
