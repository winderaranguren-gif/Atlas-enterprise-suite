...
for(const marker of ['data-global-promo-phase-ui','phaseRules','artJob:new Set([\'artwork\'])','workJob:new Set([\'materials\',\'production\'])','qcJob:new Set([\'quality_control\'])','packageJob:new Set([\'packing\'])','Approval phase required','status===\'completed\'','packageRows','workRows','artRows'])if(!commercialUi.includes(marker))throw new Error(`Phase-aware UI control missing: ${marker}`);
