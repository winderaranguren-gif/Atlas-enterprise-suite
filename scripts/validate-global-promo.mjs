...
for(const guard of ['materials_not_ready','work_order_required_before_quality','completed_work_order_required_before_quality','work_orders_incomplete','quality_check_requires_quality_control','quality_work_order_not_completed','quality_pass_required_before_packing','package_requires_packing_or_ready','package_required_before_ready','packages_not_ready','package_fulfillment_requires_ready','package_required_before_delivery','delivered_package_required','packages_not_delivered','invalid_material_transition','work_order_execution_requires_production'])if(!integrity.includes(guard))throw new Error(`Phase/workflow integrity guard missing: ${guard}`);
for(const helper of ['globalPromoMaterialTransitionAllowed','globalPromoMaterialsGate','globalPromoWorkOrdersGate','globalPromoReadyGate','globalPromoWorkOrderExecutionAllowed','globalPromoQualityAllowed','globalPromoPackageCreationAllowed','globalPromoPackageFulfillmentAllowed'])if(!integrity.includes(helper))throw new Error(`Testable workflow invariant missing: ${helper}`);
