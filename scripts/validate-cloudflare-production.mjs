...
for(const path of ['../worker/router.js','../worker/index.js','../worker/documents.js','../worker/accounting.js','../worker/backups.js']){
  if (!fs.existsSync(new URL(path, import.meta.url))) fail(`${path.replace('../','')} is missing.`);
}
...
const backups=buckets.find((entry)=>entry?.binding==='BACKUPS');
if(!backups){
  fail('R2 binding BACKUPS is missing. Configure the production backup bucket before deployment.');
}else if(!String(backups.bucket_name||'').trim()){
