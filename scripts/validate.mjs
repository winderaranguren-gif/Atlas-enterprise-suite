...
for(const marker of ["default-src 'self'","connect-src 'self'","img-src 'self' data:","media-src 'self' blob:","Permissions-Policy: camera=(), microphone=(self)","object-src 'none'","frame-ancestors 'none'",'/atlas.config.json','Cache-Control: no-store, max-age=0','/sw.js'])if(!headers.includes(marker))fail(`security header marker missing: ${marker}`);
