...
const html=read('public/index.html'),app=read('public/app.js'),core=read('public/core-services.js'),music=read('public/music-core.js'),sw=read('public/sw.js'),headers=read('public/_headers'),redirects=read('public/_redirects');
...
const browser=[app,core,music,read('public/update-core.js'),read('public/repertoire-ui.js'),read('public/surface-mode.js')].join('\n');if(/https?:\/\//i.test(browser))fail('browser runtime must not hard-code remote endpoints');
...
for(const marker of ["url.pathname.startsWith('/api/')","networkOnly(url)","fetch(request, { cache: 'no-store' })","sameOrigin(url)","'/update-core.js'","'/repertoire-ui.js'","'/surface-mode.js'"])if(!sw.includes(marker))fail(`service worker boundary missing: ${marker}`);
if(!sw.includes("const CACHE = 'atlas-core-services-v1.2.0-repertoire-app-20260810'"))fail('service worker cache version mismatch');
...
console.log(`ATLAS validation passed: v${cfg.version}, ${cfg.modules.length} production modules, ${repertoire.modules.length} repertoire surfaces, ${services.length} core services, Design Lock + Update Fabric verified.`);
