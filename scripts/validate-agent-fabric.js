...
const required=['atlas-skill-registry.js','atlas-agent-fabric.js','app.js','service-worker.js','package.json','docs/ATLAS_AGENT_FABRIC.md','.github/skills/atlas-agent-fabric/SKILL.md'];
...
const canonicalPermissions=new Set(['core.read','organization.manage','members.read','members.manage','modules.read','modules.manage','accounting.read','accounting.write','crm.read','crm.write','inventory.read','inventory.write','hr.read','hr.write','documents.read','documents.write','audit.read','identity.manage','security.events.read']);
...
for(const block of permissionBlocks){
  ...
  for(const permission of values){if(!canonicalPermissions.has(permission))fail(`skill uses non-canonical ATLAS Identity permission: ${permission}`);}
}

for(const invariant of ['Discover current state before mutation','Apply policy and permission gates','Execute only the minimum authorized action','Perform a fresh post-action verification']){
...
if(!fabric.includes('sessionStorage'))fail('Agent Fabric run history must be session-scoped in the browser');
if(fabric.includes('localStorage'))fail('Agent Fabric must not persist run history across browser sessions');
...
for(const pattern of forbidden){if(pattern.test(registry)||pattern.test(fabric))fail(`possible embedded secret matched ${pattern}`);}

console.log('ATLAS Agent Fabric validation passed: skills, canonical Identity permissions, privacy, runtime invariants, loader order, PWA cache and package validation are wired.');
