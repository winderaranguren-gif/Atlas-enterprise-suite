...
const fromEnv=String(process.env.WORKERS_CI_COMMIT_SHA||process.env.GITHUB_SHA||'').trim();
const branch=String(process.env.WORKERS_CI_BRANCH||process.env.GITHUB_REF_NAME||'main').trim()||'main';
let sha=fromEnv;
...
console.log(`ATLAS release stamped ${branch} @ ${sha}`);
