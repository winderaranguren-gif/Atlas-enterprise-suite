...
function buildPlan(input={}){
  const prompt=clean(input.prompt||input.description,8000);
  if(!prompt)return {error:'A creation brief is required.'};
  const visual=clean(input.visualReference,2000);
  const mode=inferMode(`${prompt} ${visual}`,clean(input.mode,30).toLowerCase());
  const owner=inferModule(`${prompt} ${visual}`);
  const route=safeRoute(input.route)||(mode==='website'?'/':mode==='release'?'/studio/release':null);
  ...
  return {
    service:'atlas-universal-creator',version:1,createdAt:new Date().toISOString(),externalProviders:[],
    ...
    pipeline:flow,
    handoff:{engine:MODES[mode].engine,route:MODES[mode].handoff,storageKey:'atlas.creator.universal.plan'},
...
export async function handleUniversalCreator(request){
  ...
  if(p==='/api/studio/creator/universal/health'&&request.method==='GET')return json({ok:true,service:'atlas-universal-creator',version:1,capabilities:CAPABILITIES.length});
  if(p==='/api/studio/creator/universal/capabilities'&&request.method==='GET')return json({service:'atlas-universal-creator',version:1,externalProviders:[],modes:Object.keys(MODES),modules:MODULES,capabilities:CAPABILITIES});
...
export {buildPlan as buildUniversalCreatorPlan};
