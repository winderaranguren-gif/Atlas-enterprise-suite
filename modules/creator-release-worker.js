...
const DASHBOARD_MODULES=['Dashboard','Accounting','Operations','HR & Payroll','Transportation','CRM & Sales','Inventory','Projects','Reports & Analytics','Documents','Integrations','Settings'];
const PROJECTS={
  'main-dashboard':{
    id:'main-dashboard',name:'ATLAS Main Dashboard',country:'GLOBAL',locale:'en-US',currency:'N/A',releaseKind:'web',
    route:'/',aliases:[],liveMarker:'Public preview outside. Authorized modules inside.',
    modules:DASHBOARD_MODULES,
    webBrief:{
      purpose:'Present ATLAS Enterprise Suite before sign-in and transition authorized users into the application.',
      visualReference:'Approved ATLAS split composition: Orlando brand story on the left and a dark enterprise dashboard preview on the right. Show module menus before sign-in but keep protected module routes inert until authorization.',
      route:'/',authEntry:'/identity',owner:'Platform · Public Web',modules:DASHBOARD_MODULES,
      publicBeforeSignIn:true,dataPolicy:'authorized-only',targetFile:'modules/public-dashboard-home.js',routingFile:'rideos-router.js',validationFile:'scripts/validate-public-site.mjs'
    }
  },
  venezuela:{
    id:'venezuela',name:'ATLAS Venezuela',country:'VE',locale:'es-VE',currency:'VES',releaseKind:'country',
    route:'/ve',aliases:['/venezuela','/global/ve'],liveMarker:'ATLAS Venezuela',
...
function safeProject(id){return PROJECTS[String(id||'main-dashboard').toLowerCase()]||null;}
...
function releaseContract(project){
  const gates=[
    {id:'architecture',label:'Integrated into canonical ATLAS',state:'ready'},
    {id:'routes',label:`Canonical route ${project.route} registered`,state:'ready'},
    {id:'creator',label:project.releaseKind==='web'?'ATLAS Creator Web Director':'ATLAS Creator Director + Production',state:'ready'}
  ];
  if(project.releaseKind==='web')gates.push({id:'auth-boundary',label:'Public preview → Identity → authorized modules',state:'ready'});
  if(project.releaseKind==='country')gates.push({id:'country-layer',label:`Country layer ${project.country} / ${project.locale} / ${project.currency}`,state:'ready'});
  gates.push(
    {id:'cloudflare-auth',label:'Cloudflare authentication',state:'external-check'},
    {id:'production',label:`Production route ${project.route}`,state:'live-check'}
  );
  return {
    project:{id:project.id,name:project.name,country:project.country,locale:project.locale,currency:project.currency,releaseKind:project.releaseKind,route:project.route,aliases:project.aliases,modules:project.modules},
    source:{repository:REPO,branch:'main',workflow:WORKFLOW},
    gates,
    policy:{simulateDeploy:false,requiresAuthorizedProvider:true,fabricatedMetrics:false,protectedModulesRequireAuthorization:project.releaseKind==='web'}
  };
}

function page(project){
  const modules=project.modules.map(m=>`<span class="chip">${m}</span>`).join('');
  const web=project.releaseKind==='web';
  const packageBody=web
    ? `<b>Visual target:</b> Orlando brand story + enterprise dashboard preview\n\n<b>Route:</b> ${project.webBrief.route} · Sign in: ${project.webBrief.authEntry}\n\n<b>Data:</b> authorized-only · no fabricated business metrics`
    : `<b>Hook:</b> ${project.creative.hook}\n\n<b>Formato:</b> ${project.creative.format} · ${project.creative.duration}s\n\n<b>CTA:</b> ${project.creative.cta}`;
  const planLabel=web?'Crear plan en Web Director':'Crear plan en Creator Director';
  const eyebrow=web?'RELEASE CENTER · PUBLIC MAIN':'RELEASE CENTER · COUNTRY LAUNCH';
  const releaseNote=web
    ? 'Los nombres de módulos se muestran como descubrimiento de producto antes de iniciar sesión. Las rutas internas, datos operativos y acciones permanecen protegidos hasta identidad y autorización.'
    : 'Las métricas operativas permanecen vacías hasta conectar fuentes autorizadas. Fiscalidad, banca, pagos regulados y cumplimiento se mantienen como capas sujetas a validación local y proveedores autorizados.';
  ...
  </style></head><body><main class="shell"><header class="top"><div class="brand">ATLAS CREATOR</div><div class="links"><a class="btn" href="/studio">Studio</a><a class="btn" href="/studio/director">Director</a><a class="btn" href="/studio/creator/web">Web Director</a><a class="btn" href="/studio/production">Production</a><a class="btn" href="/studio/release/main-dashboard">Main Release</a><a class="btn" href="/studio/release/venezuela">Venezuela</a></div></header>
  <section class="hero"><div><span class="eyebrow">${eyebrow}</span><h1>${project.name}</h1><p>Centro de lanzamiento de ATLAS Creator para coordinar producto, contenido y verificación de producción. Este panel no simula despliegues: consulta el estado real disponible y deja cualquier credencial o proveedor externo como una dependencia explícita.</p></div><div><span class="badge">${project.country} · ${project.locale} · ${project.currency}</span></div></section>
  <section class="grid"><article class="card"><h2>Release gates</h2><div class="gates">${releaseContract(project).gates.map(g=>`<div class="gate"><span>${g.label}</span><b class="state ${g.state==='ready'?'ready':'check'}" ${g.id==='cloudflare-auth'?'id="cfState"':g.id==='production'?'id="prodState"':''}>${g.state==='ready'?'READY':'CHECK'}</b></div>`).join('')}</div><div class="actions"><button class="btn primary" id="recheck">Revisar producción</button><a class="btn" href="${project.route}">Abrir ${project.route}</a><a class="btn" id="workflow" target="_blank" rel="noreferrer" style="display:none">Abrir workflow</a></div></article>
  ...
  <section class="grid"><article class="card"><h2>Módulos de ${project.name}</h2><div class="chips">${modules}</div><div style="height:12px"></div><div class="notice">${releaseNote}</div></article>
  <article class="card"><h2>ATLAS Creator launch package</h2><div class="brief">${packageBody}</div><div class="actions"><button class="btn gold" id="buildPlan">${planLabel}</button><button class="btn" id="copyBrief">Copiar brief</button></div></article></section>
  </main><script>
  ...
  function setState(el,label,cls){if(!el)return;el.textContent=label;el.className='state '+cls}
  async function check(){log.textContent='Consultando workflow y ruta de producción…';setState(prod,'CHECK','check');setState(cf,'CHECK','check');let lines=[];try{const r=await fetch('/api/studio/release/status?project='+encodeURIComponent(project.id),{cache:'no-store'});const s=await r.json();const g=s.github||{};lines.push('Workflow: '+(g.status||'unavailable')+(g.conclusion?' / '+g.conclusion:''));lines.push('Commit: '+(g.headSha||'unknown'));if(g.url){wf.href=g.url;wf.style.display='inline-flex'}if(g.conclusion==='success')setState(cf,'PASS','ready');else if(g.conclusion==='failure')setState(cf,'BLOCKED','fail');else setState(cf,'PENDING','check')}catch(e){lines.push('Workflow: unavailable')}
    try{const r=await fetch(project.route+'?atlas_release_check='+Date.now(),{cache:'no-store'});const text=await r.text();const live=r.ok&&text.includes(project.liveMarker);setState(prod,live?'LIVE':'NOT LIVE',live?'ready':'fail');lines.push('Production '+project.route+': '+(live?'verified':'not verified'));}catch(e){setState(prod,'UNREACHABLE','fail');lines.push('Production '+project.route+': unreachable')}log.textContent=lines.join('\n')}
  document.getElementById('recheck').onclick=check;document.getElementById('copyBrief').onclick=async()=>{const source=project.releaseKind==='web'?project.webBrief:[project.creative.hook,project.creative.script,project.creative.cta].join('\n\n');const text=typeof source==='string'?source:JSON.stringify(source,null,2);try{await navigator.clipboard.writeText(text);log.textContent='Launch brief copied.'}catch{log.textContent=text}};
  document.getElementById('buildPlan').onclick=async()=>{log.textContent='ATLAS Creator está construyendo el plan…';try{const isWeb=project.releaseKind==='web';const endpoint=isWeb?'/api/studio/creator/web/recipe':'/api/studio/director/recipe';const payload=isWeb?project.webBrief:project.creative;const r=await fetch(endpoint,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(payload)});const recipe=await r.json();if(!r.ok)throw new Error(recipe.error||'ATLAS Creator failed');if(isWeb){localStorage.setItem('atlas.creator.web.recipe',JSON.stringify(recipe));localStorage.setItem('atlas.creator.release.project',project.id);location.href='/workbench'}else{localStorage.setItem('atlas.creator.recipe',JSON.stringify(recipe));localStorage.setItem('atlas.creator.release.project',project.id);location.href='/studio/production'}}catch(e){log.textContent='ATLAS Creator: '+e.message}};check();
  </script></body></html>`;
}

export async function handleCreatorRelease(request){
  const url=new URL(request.url);const path=url.pathname.replace(/\/+$/,'')||'/';
  if(path==='/api/studio/release/projects'&&request.method==='GET')return json({service:'atlas-creator-release',projects:Object.values(PROJECTS).map(p=>({id:p.id,name:p.name,country:p.country,route:p.route,releaseKind:p.releaseKind}))});
  ...
  if(path==='/studio/release'&&request.method==='GET')return new Response(page(PROJECTS['main-dashboard']),{headers:HTML_HEADERS});
  if(path==='/studio/release/main-dashboard'&&request.method==='GET')return new Response(page(PROJECTS['main-dashboard']),{headers:HTML_HEADERS});
  if(path==='/studio/release/venezuela'&&request.method==='GET')return new Response(page(PROJECTS.venezuela),{headers:HTML_HEADERS});
