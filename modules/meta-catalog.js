...
export function atlasMetaCatalog(){return CATALOG.map(item=>{const commercialStatus=commercialStateFor(item.id);return {...item,brand:item.id==='united-hands-hub'?'United Hands for Humanity':'ATLAS',countryOfOrigin:'US',commercialStatus,approvedForSale:commercialStatus==='active',availability:metaAvailabilityFor(commercialStatus),condition:'new',link:itemLink(item),imageLink:imageLink(item)}})}
...
export async function metaCatalogRoutes(request,env,url){
 ...
 if(url.pathname==='/feeds/meta/atlas-catalog.json')return Response.json({ok:true,source:'ATLAS Enterprise Suite',currency:'USD',countryOfOrigin:'US',count:CATALOG.length,commercialPolicy:catalogCommercialPolicy(),items:atlasMetaCatalog()},{headers:{'cache-control':'public,max-age=900','access-control-allow-origin':'*'}});
 if(url.pathname==='/feeds/meta/status'){
  ...
  return Response.json({ok:true,feed:`${ORIGIN}/feeds/meta/atlas-catalog.csv`,count:CATALOG.length,mode:'scheduled-feed',credentialsRequired:false,commercialPolicy:catalogCommercialPolicy(),commercialCounts:counts},{headers:{'cache-control':'no-store'}});
 }
 return null;
}
