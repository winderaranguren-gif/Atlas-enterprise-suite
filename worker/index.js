...
const EXPECTED_TABLES=[...Object.values(CRM_TYPES),'audit_log','atlas_users','atlas_memberships','atlas_system_health','atlas_repair_log'];
...
function scopeFrom(request){return {organizationId:request.headers.get('x-atlas-organization')||'',dbaId:request.headers.get('x-atlas-dba')||'',userId:request.headers.get('x-atlas-user-id')||''}}
...
async function authorize(env,request,permission){
 ...
 if(!scope.organizationId||!scope.dbaId||!scope.userId)return {ok:false,status:401,error:'ATLAS identity and organization/DBA scope headers are required'};
 const membership=await env.DB.prepare(`SELECT m.role,m.status,u.email,u.name,u.status AS user_status FROM atlas_memberships m JOIN atlas_users u ON u.id=m.user_id WHERE m.user_id=? AND m.organization_id=? AND (m.dba_id=? OR m.dba_id='*') ORDER BY CASE WHEN m.dba_id=? THEN 0 ELSE 1 END LIMIT 1`).bind(scope.userId,scope.organizationId,scope.dbaId,scope.dbaId).first();
 if(!membership||membership.status!=='active'||membership.user_status!=='active')return {ok:false,status:403,error:'No active ATLAS membership for this organization/DBA scope'};
 if(!hasPermission(membership.role,permission))return {ok:false,status:403,error:`Permission denied: ${permission}`};
 return {ok:true,scope,identity:{userId:scope.userId,email:membership.email,name:membership.name,role:membership.role}};
}
async function audit(env,auth,action,resourceType,resourceId,payload={}){await env.DB.prepare('INSERT INTO audit_log(id,organization_id,dba_id,actor_user_id,action,resource_type,resource_id,payload,created_at) VALUES(?,?,?,?,?,?,?,?,?)').bind(id(),auth.scope.organizationId,auth.scope.dbaId,auth.scope.userId,action,resourceType,resourceId,JSON.stringify(payload),new Date().toISOString()).run()}
async function selfRepair(env){
 ...
 await setHealth(env,'identity',existing.has('atlas_users')&&existing.has('atlas_memberships')?'healthy':'blocked',{});
...
async function bootstrap(env,request){
 ...
 const auth=request.headers.get('authorization')||'';if(auth!==`Bearer ${env.ATLAS_BOOTSTRAP_TOKEN}`)return json({error:'Invalid bootstrap credential'},401);
 const body=await request.json();const email=String(body.email||'').trim().toLowerCase(),name=String(body.name||'').trim(),organizationId=String(body.organizationId||'').trim(),dbaId=String(body.dbaId||'*').trim();
 if(!email||!organizationId)return json({error:'email and organizationId are required'},400);
 const now=new Date().toISOString();let user=await env.DB.prepare('SELECT id FROM atlas_users WHERE email=?').bind(email).first();let userId=user?.id;
 ...
 return json({ok:true,userId,organizationId,dbaId,role:'owner'},201);
}
export default {
 ...
 async fetch(request,env){
  ...
  if(url.pathname==='/api/users/health'&&request.method==='GET'){await ensureCoreSchema(env);const counts=await env.DB.prepare(`SELECT (SELECT COUNT(*) FROM atlas_users) users,(SELECT COUNT(*) FROM atlas_memberships) memberships`).first();return json({operational:true,storage:'D1',service:'ATLAS Identity',...counts})}
  ...
  if(url.pathname==='/api/users'&&request.method==='POST'){
   await ensureCoreSchema(env);const auth=await authorize(env,request,'users.manage');if(!auth.ok)return json({error:auth.error},auth.status);const body=await request.json();const email=String(body.email||'').trim().toLowerCase(),role=String(body.role||'viewer');
   if(!email||!ROLE_PERMISSIONS[role])return json({error:'Valid email and role are required'},400);const now=new Date().toISOString();let user=await env.DB.prepare('SELECT id FROM atlas_users WHERE email=?').bind(email).first();let userId=user?.id;
   ...
   await env.DB.prepare(`INSERT INTO atlas_memberships(id,user_id,organization_id,dba_id,role,status,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?) ON CONFLICT(user_id,organization_id,dba_id) DO UPDATE SET role=excluded.role,status='active',updated_at=excluded.updated_at`).bind(id(),userId,auth.scope.organizationId,String(body.dbaId||auth.scope.dbaId),role,'active',now,now).run();await audit(env,auth,'grant_membership','user',userId,{email,role,dbaId:body.dbaId||auth.scope.dbaId});return json({ok:true,userId,role},201);
  }
