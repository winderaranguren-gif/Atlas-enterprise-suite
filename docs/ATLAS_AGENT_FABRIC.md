...
ATLAS Agent Fabric is the provider-agnostic orchestration layer for specialized ATLAS skills. It keeps ATLAS independent of any single cloud, model or database vendor.
...
`window.ATLASAgentFabric.registerProvider(name, adapter)` attaches model, search, database or cloud providers. The core does not hard-code a provider. A provider adapter may expose an `execute(...)` function and an optional `supports(skill, context)` predicate.
...
Browser run history is bounded and session-scoped by default. It is not a durable audit ledger. Durable production evidence belongs in ATLAS server-side audit / Work Graph infrastructure once that integration is active.
...
The repository-wide `npm run validate` chain also executes this gate. It checks the required skill set, canonical Identity permissions, execution invariants, runtime load order, Technical Support connection, package wiring and obvious embedded-secret patterns.
