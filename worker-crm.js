...
function assetBindingRequest(request,url){
  const target=new URL(url);
  // assets.directory is ./assets. Its binding root is already that directory,
  // so /assets/foo.webp must be fetched from the binding as /foo.webp.
  target.pathname=target.pathname.slice('/assets'.length)||'/';
  return new Request(target.toString(),{method:'GET',headers:request.headers,redirect:'follow'});
}
...
const DASHBOARD_ROUTES={
 ...
 '/platform/operations#workflows':'/platform/operations/workflows','/platform/operations#approvals':'/platform/operations/approvals','/platform/operations#tasks':'/platform/operations/tasks','/platform/operations#calendar':'/platform/operations/tasks','/platform/operations#reminders':'/platform/operations/tasks','/platform/operations#automation':'/platform/operations/workflows','/platform/operations#alerts':'/platform/operations',
...
export default {
  async fetch(request,env){
    ...
    const publicResponse=await publicSiteRoutes(request,env,url);
    ...
    if(url.pathname==='/platform/crm'){
      ...
      const body=crmPage().replace('</body>','<script src="/assets/atlas-module-visual.js" defer></script></body>');
