(() => {
  ...
  function tick() {
    ...
    updateMetrics();
    if (state.tick % 4 === 0) log('FUSION', 'Perception frame fused', { objects: state.objects, latency_ms: Math.round(state.latency), sensor_health: overallSensorHealth() });
    if (state.tick % 9 === 0) log('TWIN', 'Digital twin state synchronized locally', { soc: Number(state.soc.toFixed(1)), ai_load: Math.round(state.aiLoad) });
  }
  ...
  function toggleLanguage() {
    ...
    applyLanguage();
    log('UI', `Language changed to ${state.language.toUpperCase()}`);
  }
  ...
  applyLanguage();
  log('SYSTEM', 'Architecture loaded', { version: '1.0.1', actuation: 'disabled' });
})();
