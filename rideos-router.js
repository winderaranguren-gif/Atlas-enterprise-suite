...
async function surfacePlatformLinks(response){
  ...
  if(!html.includes('href="/studio/creator/visual"'))links.push('<a class="nav" href="/studio/creator/visual"><span class="ico">◫</span>Visual Inspector</a>');
  if(!html.includes('href="/studio/mission"'))links.push('<a class="nav" href="/studio/mission"><span class="ico">◇</span>Mission Control</a>');
...
export default {
  async fetch(request,env,ctx){
    ...
    if(isStudioPath(url.pathname)){
      const mission=await handleCreatorMission(request,env,ctx);if(mission)return applicationResponse(mission);
      const visual=handleCreatorVisualInspector(request,env,ctx);if(visual)return applicationResponse(visual);
