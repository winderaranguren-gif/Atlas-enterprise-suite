...
import { accessControlRoutes } from './modules/access-control.js';
...
import { inventoryCountsRoutes } from './modules/inventory-counts.js';
...
function sanitizeDashboardPlaceholders(body){if(!body.includes('ATLAS Command Center'))return body;return body
 .replace('<span>Total Revenue</span><strong>$2.45M</strong><small>▲ 12.5% vs last month</small>','<span>Receivables</span><strong>—</strong><small>Loading scoped ATLAS data</small>')
 .replace('<span>Net Profit</span><strong>$245,800</strong><small>▲ 8.2% current period</small>','<span>Payables</span><strong>—</strong><small>Loading scoped ATLAS data</small>')
 .replace('<span>Orders</span><strong>1,783</strong><small>▲ 4.7% active volume</small>','<span>Open Tasks</span><strong>—</strong><small>Loading scoped ATLAS data</small>')
 .replace('<span>Employees</span><strong>156</strong><small>● workforce online</small>','<span>Employees</span><strong>—</strong><small>Loading scoped ATLAS data</small>')
 .replace('<h2>REVENUE OVERVIEW</h2><span>Live visual preview</span>','<h2>OPERATING SNAPSHOT</h2><span>Loading scoped data</span>')
 .replace('<div class="chart-area"><div class="chart-line">','<div class="chart-area"><div class="chart-line" aria-hidden="true" style="visibility:hidden">')
 .replace('<h2>RECENT ACTIVITY</h2><span>System feed</span>','<h2>OPERATING SIGNALS</h2><span>Loading scoped data</span>')
 .replace('<div class="activity-list"><div class="event"><i class="dot"></i><div><b>Finance workspace synchronized</b><span>Reporting state updated</span></div><time>Now</time></div><div class="event"><i class="dot"></i><div><b>Operations automation ready</b><span>Workflow engine available</span></div><time>2m</time></div><div class="event"><i class="dot"></i><div><b>HR Knowledge connected</b><span>People modules available</span></div><time>8m</time></div><div class="event"><i class="dot"></i><div><b>ATLAS AI online</b><span>Assistant route operational</span></div><time>12m</time></div></div>','<div class="activity-list"><div class="event"><i class="dot"></i><div><b>Loading live ATLAS data</b><span>No placeholder activity is displayed.</span></div><time>Live</time></div></div>')}
function normalizeHtmlShell(body){let normalized=body.replace('<main class="stage" id="main">','<section class="stage" id="atlas-workspace" role="region" aria-label="ATLAS workspace">').replace('</main>\n  <aside class="ai-panel">','</section>\n  <aside class="ai-panel">');normalized=enhanceFinanceNavigation(normalized);normalized=enhanceHrNavigation(normalized);return sanitizeDashboardPlaceholders(normalized)}
async function withRuntime(response){const type=response.headers.get('content-type')||'';if(!type.includes('text/html'))return response;const source=normalizeHtmlShell(await response.text());const body=source.replace('</body>','<script src="/assets/atlas-runtime.js" defer></script><script src="/assets/atlas-dashboard-runtime.js" defer></script><script src="/assets/atlas-workspace-scope.js" defer></script></body>');const headers=new Headers(response.headers);headers.delete('content-length');return new Response(body,{status:response.status,statusText:response.statusText,headers})}
...
function securityUnavailable(){return html(`<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="noindex,nofollow"><meta name="theme-color" content="#020711"><title>ATLAS Identity</title><style>body{margin:0;min-height:100vh;display:grid;place-items:center;background:#020711;color:#eef7ff;font-family:Inter,system-ui,sans-serif}.card{max-width:620px;margin:24px;padding:28px;border:1px solid #25527a;border-radius:18px;background:#071522;box-shadow:0 22px 70px #0008}.eyebrow{color:#59c9ff;text-transform:uppercase;letter-spacing:.16em;font-size:.72rem}.card h1{margin:12px 0;font-size:2rem}.card p{color:#9fb4c7;line-height:1.6}.actions{display:flex;gap:10px;flex-wrap:wrap;margin-top:18px}.actions a{display:inline-block;padding:10px 14px;border:1px solid #2d76b4;border-radius:10px;color:#eef7ff;text-decoration:none}.actions a.primary{background:linear-gradient(135deg,#7ee6ff,#2f8cff);color:#04111d;font-weight:800}</style></head><body><main class="card"><div class="eyebrow">ATLAS IDENTITY</div><h1>Account access is not ready yet.</h1><p>ATLAS could not validate an active identity session. If this is your first time, create your ATLAS account. If you already registered, return to Sign in.</p><div class="actions"><a class="primary" href="/signup">Sign up</a><a href="/login">Sign in</a><a href="/trust/status">System status</a></div></main></body></html>`,503)}
function secureServiceWorker(){return `const C='atlas-${ATLAS_VERSION}-secure-1';const PUBLIC=['/','/login','/signup','/icon.svg','/manifest.webmanifest'];const protectedPath=p=>p==='/dashboard'||p==='/menu-experience'||p==='/settings'||p.startsWith('/platform/');self.addEventListener('install',e=>e.waitUntil(caches.open(C).then(c=>c.addAll(PUBLIC)).then(()=>self.skipWaiting())));self.addEventListener('activate',e=>e.waitUntil(caches.keys().then(keys=>Promise.all(keys.filter(k=>k!==C&&k.startsWith('atlas-')).map(k=>caches.delete(k)))).then(()=>self.clients.claim())));self.addEventListener('fetch',e=>{if(e.request.method!=='GET')return;const u=new URL(e.request.url);if(u.origin!==location.origin)return;if(u.pathname.startsWith('/api/')||protectedPath(u.pathname)){e.respondWith(fetch(e.request,{cache:'no-store'}));return}e.respondWith(fetch(e.request).then(r=>{if(!r||!r.ok)return r;const x=r.clone();caches.open(C).then(c=>c.put(e.request,x));return r}).catch(()=>caches.match(e.request).then(r=>r||caches.match('/'))))});`}
...
const accessResponse=await accessControlRoutes(request,env,url);if(accessResponse)return withRuntime(accessResponse);
...
const inventoryCountsResponse=await inventoryCountsRoutes(request,env,url);if(inventoryCountsResponse)return withRuntime(inventoryCountsResponse);
