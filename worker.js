  ...
  async fetch(request) {
    try {
      ...
      if (url.pathname === '/health') {
        return json({ status: 'ok', service: 'atlas-enterprise-suite', generation: 'clean-reset-v2' });
      }

      if (url.pathname === '/api/readiness') {
        return json({ ready: true, service: 'atlas-enterprise-suite', generation: 'clean-reset-v2' });
      }
      ...
      console.error('atlas_fetch_error', error);
      return json({
        status: 'degraded',
        service: 'atlas-enterprise-suite',
        error: 'runtime_fallback',
      }, 500);
    }
  },
