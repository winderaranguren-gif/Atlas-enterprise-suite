...
const MUSIC_CSP="default-src 'self'; script-src 'self'; connect-src 'self' https://*.supabase.co wss://*.supabase.co; style-src 'self' 'unsafe-inline'; img-src 'self' data: blob: https://*.mzstatic.com https://i.ytimg.com; font-src 'self' data:; media-src 'self' blob:; frame-src https://www.youtube.com https://www.youtube-nocookie.com; frame-ancestors 'none'; base-uri 'self'; form-action 'self'";
...
function json(payload,status=200,headers={}){
  return secure(new Response(JSON.stringify(payload),{status,headers:{'Content-Type':'application/json; charset=utf-8',...headers}}));
}
...
function requestIp(request){return String(request.headers.get('cf-connecting-ip')||'unknown').slice(0,48);}
...
export default {
  async fetch(request,env){
    ...
    const protectedApi=url.pathname.startsWith('/api/gps/')||url.pathname.startsWith('/api/support/')||url.pathname.startsWith('/api/music/');
    if(protectedApi){
      const authorization=await authorizeProtectedApi(request,url,env);
      if(authorization.response)return authorization.response;
      const base={requestId:crypto.randomUUID(),at:new Date().toISOString(),userId:authorization.user.id};
    ...
    if(['/atlas-music.html','/atlas-music.css','/atlas-music.js','/atlas-music-providers.js','/atlas-music-fixes.js'].includes(url.pathname)){
