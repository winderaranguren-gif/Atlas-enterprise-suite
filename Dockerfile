FROM node:22-alpine
WORKDIR /app
ENV NODE_ENV=production HOST=0.0.0.0 PORT=8080 ATLAS_RUNTIME_PROVIDER=container ATLAS_STATE_DIR=/var/lib/atlas
COPY package.json ./
RUN npm install --omit=dev --no-audit --no-fund
COPY . .
RUN mkdir -p /var/lib/atlas && chown -R node:node /app /var/lib/atlas
USER node
EXPOSE 8080
