...
const titles={
  home:['Command Center','Clean foundation with ATLAS-owned core services.'],
  ...
  music:['ATLAS Music','Provider-independent ATLAS Originals with optional external adapters.'],
...
function render(){
  ...
  if(state.view==='home'){
    ...
    root.innerHTML=state.config.modules.map((m,i)=>card(m,'Registered capability in the ATLAS v1.1 clean foundation.',`Module ${String(i+1).padStart(2,'0')}`)).join('');
    ...
    root.innerHTML=[card('Region preference',state.region,'Local setting'),card('Data boundary','Only UI preferences and bounded session runtime state live in the browser. Production secrets are never embedded in the client.','Security'),card('Integration gateway','External services attach through optional adapters and remain disconnected until authorized configuration exists.',serviceLabel(services.integrations)),card('Contact',state.config.contact,'Operations')].join('');
  }
}
async function boot(){
  ...
  render();if('serviceWorker'in navigator)navigator.serviceWorker.register('/sw.js').catch(()=>{});setInterval(()=>{const el=document.querySelector('#clock');if(el)el.textContent=new Date().toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'});},1000);
}
