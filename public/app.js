...
function render(){
  ...
  if(state.view==='home'){
    ...
    root.innerHTML=[card('Region preference',state.region,'Local setting'),card('Data boundary','Only UI preferences and bounded session runtime state live in the browser. Production secrets are never embedded in the client.','Security'),card('Integration gateway','External services attach through same-origin adapters and remain disconnected until an authorized health check succeeds.',serviceLabel(services.integrations)),card('Contact',state.config.contact,'Operations')].join('');
  }
}
async function boot(){
