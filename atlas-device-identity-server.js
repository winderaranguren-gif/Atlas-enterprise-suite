...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'POST' && pathname === '/api/identity/challenges') {
    ...
    challenges.set(id, {
      id,
      phoneTokenHash: crypto.createHash('sha256').update(phoneToken).digest('hex'),
      pollTokenHash: crypto.createHash('sha256').update(pollToken).digest('hex'),
      createdAt,
    ...
    const phoneUrl = `${publicBase(req)}/atlas-device-identity.html?mode=phone&challenge=${encodeURIComponent(id)}&token=${encodeURIComponent(phoneToken)}`;
  ...
  if (req.method === 'GET' && stateMatch) {
    ...
    if (!item || item.expiresAt <= now()) return sendJson(res, 404, { ok: false, error: 'challenge_not_found_or_expired' });
    const token = req.headers.authorization?.replace(/^Bearer\s+/i, '') || '';
    const hash = crypto.createHash('sha256').update(token).digest('hex');
    if (!token || hash !== item.pollTokenHash) return sendJson(res, 403, { ok: false, error: 'invalid_poll_token' });
  ...
  if (req.method === 'POST' && captureMatch) {
    ...
    try {
      ...
      const hash = crypto.createHash('sha256').update(token).digest('hex');
      if (!token || hash !== item.phoneTokenHash) return sendJson(res, 403, { ok: false, error: 'invalid_phone_token' });
  ...
  if (req.method === 'GET' && (pathname === '/' || pathname === '/atlas-device-identity.html')) {
    ...
    return fs.readFile(file, (error, data) => {
      ...
      res.writeHead(200, {
        ...
        'Cache-Control': 'no-store',
        'Content-Security-Policy': "default-src 'self'; img-src 'self' data:; media-src 'self' blob:; script-src 'self'; style-src 'self' 'unsafe-inline'; connect-src 'self'"
      });
      res.end(data);
    });
  }
...
server.listen(PORT, HOST, () => {
  ...
  console.log('Phone and laptop must reach this server over the same trusted network or HTTPS tunnel.');
