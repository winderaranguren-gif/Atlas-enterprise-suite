...
const required=[
  ...
  'atlas/portable-runtime.mjs','atlas/local-durable.mjs','atlas/video-signal-server.mjs','atlas/server.mjs',
  'api/index.mjs','vercel.json','Dockerfile','scripts/validate-portable-runtime.mjs','scripts/validate-portable-video.mjs'
];
...
for(const f of ['package.json','vercel.json']){
  try{JSON.parse(await readFile(resolve(ROOT,f),'utf8'));checks.push({name:`json:${f}`,ok:true});}catch(e){checks.push({name:`json:${f}`,ok:false,error:e.message});}
}
